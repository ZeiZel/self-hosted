...
# Настройка Pangolin и Wireguard туннеля
...
Запустите playbook для развёртки клиента:
...
## Проверка Wireguard подключения

После регистрации клиента проверьте статус Wireguard туннеля.

### На сервере

Проверьте статус Wireguard на сервере:
...
Проверьте статус Wireguard на клиенте:
...
Проверьте ping до сервера через туннель:
...
## Настройка маршрутизации портов и доменов

После успешного подключения клиента, можно настроить маршрутизацию портов и доменов.

### Настройка через веб-интерфейс Pangolin

1. Войдите в веб-интерфейс Pangolin
2. Перейдите в раздел настройки маршрутизации
...
Traefik на сервере автоматически настроен для проксирования запросов через туннель. Убедитесь, что:
...
## Настройка SSH туннелей через Pangolin
...
Проверьте логи на локальной машине:
...
Убедитесь, что домены правильно настроены в DNS или `/etc/hosts`.

### Проблема: Сервисы недоступны через туннель

Проверьте маршрутизацию в Pangolin:

1. Войдите в веб-интерфейс Pangolin
2. Проверьте правила маршрутизации
3. Убедитесь, что сервисы доступны локально
4. Проверьте логи Traefik на сервере

### Проблема: Ошибки SSL сертификатов

Проверьте настройки Let's Encrypt:

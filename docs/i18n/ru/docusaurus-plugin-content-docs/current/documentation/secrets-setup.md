...
Проверьте список ваших GPG ключей:
...
Убедитесь, что файл `.sops.yaml` в корне проекта содержит ID вашего GPG ключа:
...
Убедитесь, что переменная `GPG_TTY` установлена:
...
## Структура секретов для каждого сервиса
...
### Проблема: Не могу расшифровать файл

Проверьте, что ваш GPG ключ доступен:

```bash
gpg --list-secret-keys
```

Проверьте, что ключ указан в `.sops.yaml`:

```bash
cat .sops.yaml
```

### Проблема: GPG_TTY не установлена
...
### Проблема: SOPS не может найти ключ
...
Убедитесь, что ваш ключ указан в метаданных.

    ...
    .action(createCommandAction(
      ...
      async ({ configService, inventoryService, servicesService, tableService }, options) => {
        ...
        logger.keyValue({
          ...
          'Project directory': configService.hasValidRepo() ? configService.getProjectDir() : chalk.gray('N/A'),
          'Repository': configService.hasValidRepo() ? configService.getRepoRoot() : chalk.red('Not found'),
        });
